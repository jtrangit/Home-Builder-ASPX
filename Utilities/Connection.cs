...
namespace Utilities
{
    public class DBConnect
    {
        ...
        String SqlConnectString = "server=cis-mssql1.temple.edu;Database=sp24_3342_tun45633;User id=tun45633;Password=Keem4desif";

        // Home Connection String - used for working from home using SSH Tunneling.
        //String SqlConnectString = "server=127.0.0.1,5555;Database=sp24_3342_tun45633;User id=tun45633;Password=Keem4desif";
        ...
        //update homesales record 
        public int UpdateHomeSales(decimal totalCost, string homePlan)
        {
            string updateQuery = "UPDATE HomeSales SET [Quantity Sold] = [Quantity Sold] + 1, [Total Revenue] = [Total Revenue] + @TotalCost WHERE Homeplan = @Plan";
            using (SqlConnection connection = new SqlConnection(SqlConnectString))
            {
                using (SqlCommand theCommand = new SqlCommand(updateQuery, connection))
                {
                    theCommand.Parameters.AddWithValue("@TotalCost", totalCost);
                    theCommand.Parameters.AddWithValue("@Plan", homePlan);
                    connection.Open();
